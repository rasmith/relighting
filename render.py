...
class Sampler(object):
    def __init__(
        ...
        self.lookat = gm.lookat(
            self.eye.transpose(), self.at.transpose(), self.up.transpose()
        )
...
def call_embree2(data):
    shader, lock, d, i, j, samples, infile, outfile, isa, num_threads, verbose = (
        data
    )
    ...
    args = [
        ...
        str(spp),
        "--shader",
        shader_name_to_shader_arg[shader],
        "--verbose",
        str(verbose),
    ]
...
def get_output_file(shader, i, j):
    return f"{args.out_dir}/out-{i:04d}-{j:04d}.ppm"
    if shader == "default":
...
def render_update_thread(l, d, start, stdscr):
    ...
    while not done:
        ...
        if d["changed"] == True:
            ...
            # message = f'{outfile} rendered in {t:1.2f}
            # seconds {p:3.2f}% {sec_per_image:2.2f} s/img'
            ...
            stdscr.addstr(9, 0, f"ex={ex}")
            stdscr.addstr(11, 0, f"elapsed = {elapsed_str}")
            stdscr.addstr(13, 0, f"cmd={cmd}")
            if d["count"] == d["total"]:
                done = True
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_up_samples", type=int, default=1, dest="num_up_samples"
    )
    parser.add_argument(
        "--num_right_light_samples",
        type=int,
        default=1,
        dest="num_right_light_samples",
    )
    parser.add_argument(
        "--num_up_light_samples",
        type=int,
        default=1,
        dest="num_up_light_samples",
    )
    args = parser.parse_args()
    print(args.scene)
    print(args.out_dir)
    print(args.shader)
    print(args.num_right_samples)
    ...
    # num_up_light_samples = int(sys.argv[4])
    # num_right_light_samples = int(sys.argv[5])

    num_up_samples = args.num_up_samples
    num_right_samples = args.num_right_samples
    num_up_light_samples = args.num_up_light_samples
    num_right_light_samples = args.num_right_light_samples

    num_light_samples = num_up_light_samples * num_right_light_samples
    ...
    for eye, at, up in camera_sampler:
        up_light_rotation_speed = 2.0 * np.pi / num_up_light_samples
        right_light_rotation_speed = 2.0 * np.pi / num_right_light_samples
        light_sampler = Sampler(
            ...
            num_up_light_samples,
            num_right_light_samples,
            up_light_rotation_speed,
            right_light_rotation_speed,
            "light",
        )
        # light  =  np.array([[61.4156, 102.11, -1325.25]], dtype = np.float)
        samples[i] = {}
        for j, current_light in enumerate(light_sampler):
            # for j in range(num_light_samples):
    ...
    d["changed"] = False
    d["total"] = float(
        camera_sampler.total_samples * light_sampler.total_samples
    )
    d["outfile"] = ""
    lock = manager.Lock()
    ...
    num_images = camera_sampler.total_samples * light_sampler.total_samples
    ...
    print(
        "total time = %f, num_images = %d, time per image = %f"
    ...
    if shader == "default":
        json.dump(
            samples,
            codecs.open("config.cfg", "w", encoding="utf-8"),
            separators=(",", ":"),
            sort_keys=True,
            indent=4,
        )
